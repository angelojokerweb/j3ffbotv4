...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: BRUXINHO MODS\n'
            + 'ORG:Owner BRUXINHO;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5564992606313:+55 64 99260-6313\n'
            + 'END:VCARD'

prefix = "."
name = "~ ÂNGELO  | BOTS"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM TEMENTE ÂNGELO BOTS😁"
rmenu = "ÂNGELO DOMINA KARAY:)"
limitt = 10
ban = []
userpremium = ["556492606313@s.whatsapp.net"] //ubah nomer kalian
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '*desculpe, esta Pergunta só pode ser usada em grupos!*',
					benned: '*desculpe, seu número está banido, por favor, contate o dono para abrir banidos*',
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do Ângelo digite ${prefix}daftar`,
			...
			const ownerNumber = ["556492606313@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do Ângelo :D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'owner':
                 ...
                 client.sendMessage(from, 'Salve o número do meu criador ae:)',text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				if (isBanned) return reply(mess.only.benned)    
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('556492606313@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 J3FF BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sucesso deleta todo o chat ÂNGELO BOTS\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *ÂNGELO BOTS BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ÂNGELO BOTS]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
